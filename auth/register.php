...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $npm = trim($_POST['npm'] ?? '');
    $prodi = trim($_POST['prodi'] ?? ''); // Ambil prodi
    ...
    // Validasi sederhana
    if (!$nama || !$npm || !$prodi || !$email || !$password || !$password_confirm || !$id_pembimbing || !$id_penguji) {
    ...
    if (!$error) {
        try {
            ...
            $stmtInsertUser = $pdo->prepare("INSERT INTO users (nama, npm, prodi, email, password, role, created_at) VALUES (:nama, :npm, :prodi, :email, :password, 'peserta', NOW())");
  ...
  <div class="card mx-auto" style="max-width: 500px;">
    ...
    <form method="POST" action="" class="needs-validation" novalidate autocomplete="off">
      ...
      <div class="mb-3">
        ...
        <input type="text" id="prodi" name="prodi" class="form-control" required value="<?= htmlspecialchars($_POST['prodi'] ?? '') ?>" />
